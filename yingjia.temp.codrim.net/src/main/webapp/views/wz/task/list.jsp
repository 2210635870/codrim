...
<script type="text/javascript">
    ...
    function taskActionformater(value, row, index) {
    	...
    	if (row.taskStatus == "暂停") {
    		...
    		var stDate = dateStrParser(row.taskStartDate);
    		var edDate = dateStrParser(row.taskEndDate);
    		if( curDate >= stDate && curDate <= edDate ) {
    			actionStr += " | <a href='javascript:void(0)' onclick=\"online('"+row.taskId+"')\">运行</a>";
    		} else{
    			actionStr += " | <a href='javascript:void(0)' onclick=\"online('no')\">运行</a>";
    		}
    ...
    function online(taskId) {
    	if(taskId == 'no') {
    		$.messager.alert('运行失败', '投放时间已过，请重新编辑运行或下线', 'warning');
    		return;
    	}

		...
		<form:form commandName="taskReview" id="taskReviewForm" method="post" action="/wz/review/reviewTask.do">
			...
			<table cellpadding="5">
				...
				<tr>
					...
					<td>
						<c:choose>
							<c:when test="${taskReview.reviewStatus eq 2}">通过</c:when>
							<c:otherwise>不通过</c:otherwise>
						</c:choose>
					</td>
				</tr>
				<tr>
					<td class="field-name">审核日期:</td>
					<td>
						<fmt:formatDate value="${taskReview.reviewDate}" pattern="yyyy-MM-dd"/>
					</td>
				</tr>
				<tr>
					<td class="field-name">备注:</td>
					<td>
						${taskReview.reviewRemark}
					</td>
				</tr>
			</table>

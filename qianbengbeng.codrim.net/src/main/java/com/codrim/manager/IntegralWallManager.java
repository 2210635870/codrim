...
public class IntegralWallManager {
	...
	public IntegralWallResultBean resolveYouMi(HttpServletRequest request,
		...
		logger.info("有米回调 参数：订单id" + order + "|| 开发者应用id" + app + "||用户id"
				+ userId + "||渠道号" + chn + "||广告名" + ad + "||积分" + points);
		String sign = this
				.MD5(dev_server_secret + "||" + order + "||" + app + "||"
						+ userId + "||" + chn + "||" + ad + "||" + points)
				.substring(12, 20).toLowerCase();
		if (sign.equals(sig) && sig != null) {
			TbWzBinding existedBinding = userService.getBindingByDevice(userId);
			if (existedBinding != null) {
				TbWzUser user = userService.getUserById(existedBinding
						.getUserId());
	...
	public IntegralWallResultBean resolveWanPu(HttpServletRequest request,
		...
		logger.info("万普回调 参数：订单id" + order_id + "|| 开发者应用id" + app_id
				+ "||用户id" + key + "||广告名" + ad_name + "||积分" + points);
		if (!bill.equals("0") && !points.equals("0") && status.equals("1")) {
			...
			String plaintext = adv_id + app_id + key + udid + bill + points
					+ activate_time + order_id + call_back_Key;
			...
			if (keys.equalsIgnoreCase(wapskey)) {
				// 成功接收数据
				logger.info("验证成功");
				TbWzBinding existedBinding = userService
						.getBindingByDevice(key);
				if (existedBinding != null) {
					TbWzUser user = userService.getUserById(existedBinding
							.getUserId());
					user.setBalance(user.getBalance() + Long.parseLong(points));
	...
	public IntegralWallResultBean resolveDomob(HttpServletRequest request,
		...
		String queryString = request.getQueryString();
		logger.info(queryString);
		String action = request.getParameter("action");// 整型
														// 动作编号，从0开始。通常0表示激活，1表示签到
		String action_name = this.decoder(request.getParameter("action_name"));// 字符串
																				// 动作名称，例如“激活”，“签到-1”，“签到-2”等。
		...
		String orderid = this.decoder(request.getParameter("orderid"));// 字符串
																		// 订单ID。该值是唯一的，如果开发者接收到相同的订单号，那说明该订单已经存在。
		String pkg = this.decoder(request.getParameter("pkg"));// 字符串
																// 被激活的广告的包名，例如
																// cn.domob.demo
		String point = request.getParameter("point");
		String price = request.getParameter("price");// 开发者获得的收入
		String pubid = this.decoder(request.getParameter("pubid"));// Publisher
																	// ID，即开发者的积分墙应用ID
		String ts = request.getParameter("ts");// 成功结算的时间戳，精确到秒
		String user = this.decoder(request.getParameter("user"));// android平台，如果开发者没有在积分墙android
																	// SDK中绑定userid，则user为设备的imei
		String sig = this.decoder(request.getParameter("sign"));
		String sign = this.MD5("action=" + action + "action_name="
				+ action_name + "ad=" + ad + "adid=" + adid + "channel="
				+ channel + "device=" + device + "orderid=" + orderid + ""
				+ "pkg=" + pkg + "point=" + point + "price=" + price + "pubid="
				+ pubid + "ts=" + ts + "user=" + user + ""
				+ duomeng_private_key);
		logger.info("多盟回调 参数：订单id" + orderid + "|| 开发者应用id" + pubid + "||用户id"
				+ user + "||渠道号" + channel + "||广告名" + ad + "||积分" + point);
		if (sign.equalsIgnoreCase(sig) && sig != null) {
			TbWzBinding existedBinding = userService.getBindingByDevice(user);
			if (existedBinding != null) {
				TbWzUser tbuser = userService.getUserById(existedBinding
						.getUserId());
	...
	public IntegralWallResultBean resolveDianjoy(HttpServletRequest request,
		...
		String trade_type = request.getParameter("trade_type");// 表示广告任务的类型
																// 1-安装激活任务
																// 4-次日打开深度任务
		// *注意：同一广告可产生多次任务，并产生更多收入
		String sign = this.MD5(time_stamp + call_back_Key);
		logger.info("点入回调 参数：|| 开发者应用id" + app_id + "||用户id" + user + "||广告名"
				+ ad_name + "||积分" + currency);
		if (sign.equalsIgnoreCase(token) && token != null) {
			...
			if (existedBinding != null) {
				TbWzUser tbuser = userService.getUserById(existedBinding
						.getUserId());
				tbuser.setBalance(tbuser.getBalance()
						+ Long.parseLong(currency));
				userService.modifyUser(tbuser);
				bean.setSuccess(true);
				logger.info("更新成功！");
			} else {
	...
	public IntegralWallResultBean resolveChukong(HttpServletRequest request,
		...
		String queryString = request.getQueryString();
		logger.info(queryString);
		String os = request.getParameter("os");// 必填项,系统类型。定义enum: ['iOS',
												// 'Android']
		...
		String transactionid = this.decoder(request
				.getParameter("transactionid"));// 必填项,用于识别是否会用重复的积分返还请求，同一个任务的积分返还id不变
		...
		logger.info("触控回调 参数：订单id" + adid + "||用户id" + user + "||广告名"
				+ taskname + "||积分" + coins);
		if (sign.equalsIgnoreCase(sig) && sig != null) {
			TbWzBinding existedBinding = userService.getBindingByDevice(user);
			if (existedBinding != null) {
	...
	public IntegralWallResultBean resolveAnwo(HttpServletRequest request,
		...
		String token = this.MD5("adid=" + adid + "adname=" + adname
				+ "androidid=" + androidid + "appid=" + appid + "device="
				+ device + "" + "imei=" + imei + "point=" + point + "ts=" + ts
				+ "key=" + anwo_Key);
		logger.info("安沃回调 参数：|| 开发者应用id" + appid + "||用户id" + keyword + "||广告名"
				+ adname + "||积分" + point);
		if (token.equalsIgnoreCase(sign) && sign != null) {
			TbWzBinding existedBinding = userService
					.getBindingByDevice(keyword);
			if (existedBinding != null) {
				TbWzUser tbuser = userService.getUserById(existedBinding
						.getUserId());
				tbuser.setBalance(tbuser.getBalance() + Long.parseLong(point));
	...
	public IntegralWallResultBean resolveMiDi(HttpServletRequest request,
			IntegralWallResultBean bean) {
		String queryString = request.getQueryString();
		logger.info(queryString);
		...
		String cash = request.getParameter("cash");// 此次操作赚取的积分数量；
													// cash=广告价格×应用和汇率
		String imei = request.getParameter("imei");// imei
		String param0 = request.getParameter("param0");// 应用通过SDK上传的自定义参数
		String sign = request.getParameter("sign");
		StringBuilder sb = new StringBuilder(); // 签名字符串
		sb.append(id).append(trandNo).append(cash)
				.append(param0 == null ? "" : param0).append(midi_Key);
	...
	public IntegralWallResultBean resolveQiDian(HttpServletRequest request,
			IntegralWallResultBean bean) throws UnsupportedEncodingException {
		String queryString = request.getQueryString();
		logger.info(queryString);
		...
		sb.append("order=").append(order).append("app=").append(app)
				.append("adsid=").append(adsid).append("imei=").append(imei)
				.append("mac=").append(mac).append("android_id=")
				.append(android_id).append("user=").append(call_back_Key)
				.append("price=").append(price).append("point=").append(point)
				.append("task=").append(task).append("other=").append(other);
	...
	public IntegralWallResultBean resolveYiJiFen(HttpServletRequest request,
			IntegralWallResultBean bean) throws UnsupportedEncodingException {
		String queryString = request.getQueryString();
		logger.info(queryString);

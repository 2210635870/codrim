...
var  advertIdWithPutChannel;
function addPutChannel(){
	deleteAll();
	var row= $('#view').datagrid('getSelected');
	if(row){
		if(row.evaluateStatus!='商务评估'){
			alert("没有进行最终评估是否投放，不能进行渠道信息操作！");
			return ;
		}
		advertIdWithPutChannel=row.id;
		clearPutChannel();
	$('#dlg_put_channel').dialog('open').dialog('setTitle', '渠道询价');
	$('#dlg_put_channel').window('center');
...
<table  >
    <tbody id="budgeNumWithDay">
		...
		function checkEffectCallBack(type){
		 ...
		 var chk_value ='';  
		  var channelCodrimCallback='Http://test.codrim.net/rankClick?';
		  var checkedName='';
	    function getCheckBoxValue(){  //jquery获取复选框值    
	    	  $('input[name="codrimParam"]:checked').each(function(){    
	    	   ...
	    	   channelCodrimCallback=channelCodrimCallback+text.substring(text.indexOf("_")+1,text.length)+"={}&";
	    	  });    
	    	  chk_value=chk_value.substring(0,chk_value.length-1);
	    	  channelCodrimCallback=channelCodrimCallback.substring(0,channelCodrimCallback.length-1);

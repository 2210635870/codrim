$(function(){
    ...
    var jsonstr = '{"total":1,"rows":['+json+']}';  
    var data = $.parseJSON(jsonstr);   
    $("#view").datagrid("loadData",data);
    $("#view").datagrid("selectRow",0);
    
    $('#view_detail').datagrid({
        url : projectPath + '/getProductDetails.do?productName=' +data.rows[0].productName+"&customerName="+data.rows[0].customerName+"&deviceplate="+data.rows[0].deviceplate,
...
function addProductCreate(){
	var row = $('#view').datagrid('getSelected');
	if (row) {
		$('#fm').form('load', row);
		$('#addDetail').css("display","block");
		$("#submit").attr("onclick","saveProductCreate()");
		id=row.id;
...
function editProductCreate(){
	var row = $('#view_detail').datagrid('getSelected');
	if (row) {
		$('#fm').form('load', row);
		$('#addDetail').css("display","block");
		$("#submit").attr("onclick","modifyProductCreate()");
		id=row.id;
		channelNameByedit=row.channelName;
		 initChannel("fm #channelName","fm #personInCharge");
		//	getAllChannelContactName(row.channelContactName,"/getChannelNameByContactName.do?contactName=","fm #channelContactName","fm #channelName");
...
function saveProductCreate(){
	$.ajax({
		            type: "post",
		            url: projectPath+"/addProductDetail.do",
		            data: {
			row:json,
			deviceplate:devicePlate,
			userName:userName,
